...
CREATE TABLE student(
    ...
    -- Plan choice: flex users have a declining balance of 525 but can purchase 
    -- bar-coded items. anytime users have unlimited funds but can only buy
    -- non-bar-coded items and can only get one of each item type in one order.
    plan          ENUM('flex', 'anytime') NOT NULL DEFAULT 'flex',
    -- Total amount spent on items not included in the student meal plan. This
    -- amount is charged to the student's bursar account. 
    total_charges NUMERIC(6, 2) NOT NULL DEFAULT 0
);
...
CREATE TABLE rd_order(
    ...
    order_total  NUMERIC(5, 2) NOT NULL DEFAULT 0.0,
